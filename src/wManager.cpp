...
void init_WifiManager()
{
    ...
    WiFiManagerParameter nostr_npubtext_box("Nostr Private Key", "Your Nostr Private Key", Settings.NostrPrivatekey, 80);

    WiFiManagerParameter nostr_nsectext_box("Nostr Public Key", "Your Nostr Public Key", Settings.NostrPubkey, 80);
  ...
  wm.addParameter(&nostr_npubtext_box);
  wm.addParameter(&nostr_nsectext_box);
  wm.addParameter(&time_text_box_num);
    ...
    if (forceConfig)    
    {
        ...
        if (wm.startConfigPortal(DEFAULT_SSID, DEFAULT_WIFIPW))
        {
            ...
            Settings.NostrPrivatekey = nostr_nsectext_box.getValue();
            Settings.NostrPubkey = nostr_npubtext_box.getValue();
    ...
    if (WiFi.status() == WL_CONNECTED) {
        ...
        Serial.print("NostrString: ");
        Serial.println(Settings.NostrPrivatekey);
