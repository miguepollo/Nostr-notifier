...
bool nvMemory::saveConfig(TSettings* Settings)
{
    if (init())
    {
        ...
        json[JSON_SPIFFS_KEY_NOSTR_PRIVATE_KEY] = Settings->NostrPrivatekey;
        json[JSON_SPIFFS_KEY_NOSTR_PUBKEY] = Settings->NostrPubkey;
    ...
    if (init())
    {
        if (SPIFFS.exists(JSON_CONFIG_FILE))
        {
            ...
            if (configFile)
            {
                ...
                if (!error)
                {
                    Settings->NostrPrivatekey = json[JSON_SPIFFS_KEY_NOSTR_PRIVATE_KEY ] | Settings->NostrPrivatekey;
                    Settings->NostrPubkey = json[JSON_SPIFFS_KEY_NOSTR_PUBKEY ] | Settings->NostrPubkey;
